...
router.post('/users/id', async (request, response, next) => {
	await service.findByIdUser(request.body)
	.then(data => {
		response.status(200).send(data);
	}).catch(err => {
		console.log(err);
...
router.post('/users', async (request, response, next) => {
	const userRegister = await service.createUser(request.body)
	.then(data => {
...
router.put('/users', async (request, response, next) => {
	await service.updateUser(request.body)
	.then(data => {
...
router.delete('/users', async (request, response, next) => {
	await service.deleteUser(request.body)
	.then(data => {
